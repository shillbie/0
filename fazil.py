...
import time, random, sys, ast, re, os, io, json, subprocess, threading, string, codecs, requests, ctypes, urllib, urllib2, urllib3, wiclpedia, tempfile
...
cl = LINETCR.LINE()
cl.login(token="EoV2bUdJ9ydCnJAcWry7.Kzlr3Dnbzp+L0i77PE8AvW.Qvzac7FpwokUpTCVXjo1oux1ZT7f4pBfvMrCLEHuryc=")
cl.loginResult()

#cl = LINETCR.LINE() # Assist
...
#--------------NOTIFIED_INVITE_INTO_GROUP----------------
	    ...
	    if mid in op.param3:
			...
			cl.cancelGroupInvitation(op.param1, [op.param3])
			cl.sendText(op.param1, "Itu clcker jgn di invite!")
		    else:
			pass
		...
		if op.type == 17:
			if mid in op.param3:
				if wait["blacklist"] == True:
					...
					cl.clckoutFromGroup(op.param1,[op.param2])
					kk.clckoutFromGroup(op.param1,[op.param2])
					ks.clckoutFromGroup(op.param1,[op.param2])
		if op.type == 32:
			if mid in op.param3:
				wait["blacklist"][op.param2] == True
		if op.type == 32:
...
#--------------------------NOTIFIED_UPDATE_GROUP---------------------
		...
		if op.param2 in admin:
		    pass
                    ...
                    cl.sendText(msg.to, "Jangan mainan QR ntr ada clcker")
            else:
                pass
#--------------------------SEND_MESSAGE---------------------------
        if op.type == 25:
...
#//commandnya
		...
		cl.sendText(msg.to,"mamvos...!!!")
                ...
                msg.contentMetadata = {'mid': "u5baffcc81a0a0689982216a005cfc70b"}
                cl.sendMessage(msg)
		cl.sendText(msg.to,"Itu Yang Bicln BOT")
#--------------------------------------------------------
...
#----------------------
                ...
                jawab = ("Ya","Bisa jadi","Mungcln","Coba tanya lagi :v","Tidak")
                jawaban = random.choice(jawab)
...
#==================================================
            ...
            elif 'wicl ' in msg.text.lower():
              if msg.from_ in admin:
                  try:
                      wicl = msg.text.lower().replace("wicl ","")
                      wiclpedia.set_lang("id")
                      pesan="Title ("
                      pesan+=wiclpedia.page(wicl).title
                      pesan+=")\n\n"
                      pesan+=wiclpedia.summary(wicl, sentences=1)
                      pesan+="\n"
                      pesan+=wiclpedia.page(wicl).url
...
#--------------------------------------------------------
		...
		wait["Autoclck"] = True
		cl.sendText(msg.to,"Autoclck Active")

	    elif "Autoclck:off" in msg.text:
		wait["Autoclck"] = False
		cl.sendText(msg.to,"Autoclck inActive")
#--------------------------------------------------------
...
#--------------------------------------------------------
		...
		if wait["Autoclck"] == True: md+="◑ Autoclck : on\n"
		else:md+="◑ Autoclck : off"
...
#--------------------------------------------------------
		...
		if midd not in admin:
		    cl.clckoutFromGroup(msg.to,[midd])
		else:
		    cl.sendText(msg.to,"Admin Detected")
#--------------------------------------------------------
...
#--------------------------------------------------------
                ...
                if msg.toType == 2:
                    ...
                    for jj in matched_list:
                        cl.clckoutFromGroup(msg.to,[jj])
                    cl.sendText(msg.to,"Blacklist emang pantas tuk di usir")
#--------------------------------------------------------
            elif "clckall" in msg.text:
                if msg.toType == 2:
        ...
        if op.type == 19:
            try:
                if op.param3 in mid:
                    ...
                    if op.param2 in OWN:
                        clcker1 = [cl,cl,kk,ks,kc,ka,km,kn,ko]
                        G = random.choice(clcker1).getGroup(op.param1)
                        G.preventJoinByTicket = False
                        random.choice(clcker1).updateGroup(G)
                        cl.acceptGroupInvitationByTicket(op.param1,Ticket)
                        cl.acceptGroupInvitationByTicket(op.param1,Ticket)
                        G.preventJoinByTicket = True
                        random.choice(clcker1).updateGroup(G)
                        ...
                        random.choice(clcker1).clckoutFromGroup(op.param1,[op.param2])

                        G.preventJoinByTicket = False
                        random.choice(clcker1).updateGroup(G)
                        Ticket = random.choice(clcker1).reissueGroupTicket(op.param1)
                        cl.acceptGroupInvitationByTicket(op.param1,Ticket)
                        G.preventJoinByTicket = True
                        random.choice(clcker1).updateGroup(G)
