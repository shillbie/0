...
cl = LINETCR.LINE()
#cl.login(token="cl = LINETCR.LINE()
cl.login(token="EokJNvRPTwQtgymKlZDb.rKdr/2WyUJYmSIu//JGqAW.1dl3aTU4u+0C56+LJ/HpuAn1EI8BVu0vrTSwQ109o2c=")
cl.loginResult()
...
╠═════════════════════════
║             💠 By:🔝💦ĐøŁ💦🔝
║    〽️ http://line.me/ti/p/GQtqWr3e2m
╚═════════════════════════
"""
#----------------------------------------------------------
botMessage ="""
...
╠═════════════════════════
║THANKS FOR:
║T⃟e⃟a⃟m⃟ b⃟o⃟t⃟ A⃟m⃟a⃟t⃟i⃟r⃟    |=|=|=|   S⃟t⃟a⃟r⃟ bB⃟o⃟t⃟
╚═════════════════════════
"""
#----------------------------------------------------------
helpMessage ="""
...
admin= ["u5baffcc81a0a0689982216a005cfc70b","u246cb4d9c8a981d4299b7ef16cad49d9",mid] 
...
wait = {
    ...
    "comment4":"Nice Kak (^_^)",
    "comment5":"Bot Auto Like ©By : Insan",    
    "commentOn":True,
    "commentBlack":{},
...
#------------------NOTIFIED_clCKOUT_FROM_GROUP-----------------
        if op.type == 19:
		if wait["Autoclck"] == True:
		    try:
			if op.param3 in Bots:
			    pass
...
#--------------------------NOTIFIED_UPDATE_GROUP---------------------
		...
		if op.param2 in Bots:
		    pass
                    ...
                    cl.updateGroup(X)
                    Ti = cl.reissueGroupTicket(op.param1)
                    cl.acceptGroupInvitationByTicket(op.param1,Ti) #clcker join
                    X.preventJoinByTicket = True
                    cl.updateGroup(X)
                    cl.clckoutFromGroup(op.param1,[op.param2])
...
#--------------------------------------------------------
	    elif "Autoclck on" in msg.text:
		     wait["Autoclck"] = True
		     cl.sendText(msg.to,"Auto clck Sudah Aktif")

	    elif "Autoclck off" in msg.text:
		     wait["Autoclck"] = False
...
#--------------------------------------------------------
		...
		if wait["Autoclck"] == True: md+="╠➩✔️ Auto clck : On\n"
...
#--------------------------------------------------------
                ...
                cl.inviteIntoGroup(msg.to,[midd])
#--------------------------------------------------------
            elif "Tebakan dong" in msg.text:
                midd = "ufad6050bc087e1492034e7cdfb061320"
                cl.findAndAddContactsByMid(midd)
                cl.inviteIntoGroup(msg.to,[midd])
                
            elif "Unyu dong" in msg.text:
                midd = "u944c5e70a6302e1a2fe0900c864691d4"
                cl.findAndAddContactsByMid(midd)
                cl.inviteIntoGroup(msg.to,[midd])    
#--------------------------------------------------------
...
#--------------------------------------------------------
		...
		if msg.from_ in admin:
		    for i in gid:
			cl.sendText(i,""+bc+"\n\nContact Me : http://line.me/ti/p/GQtqWr3e2m")
...
#--------------------------------------------------------
                ...
                if msg.toType == 2:
                    ...
                    if wait["lang"] == "JP":
                        ...
                        cl.sendText(msg.to,"Not for use less than group")
#--------------------------------------------------------
            elif msg.text in ["Melebu","Join all"]:
		if msg.from_ in admin:
...
#--------------------------------------------------------
		...
		if msg.from_ in Creator:
		    ...
		    cl.sendText(msg.to,"jangan usir aku dong kak... T_T")
#--------------------------------------------------------
            elif msg.text in ["Absen"]:
		cl.sendText(msg.to,"Pasukan Absen!!")
                cl.sendText(msg.to,"TC1 Hadiir  \(ˆ▿ˆ)/")
                cl.sendText(msg.to,"TC2 Hadiir  \(ˆ▿ˆ)/")
                cl.sendText(msg.to,"TC3 Hadiir  \(ˆ▿ˆ)/")
                ko.sendText(msg.to,"TC4 Hadiir  \(ˆ▿ˆ)/")
                ku.sendText(msg.to,"TC5 Hadiir  \(ˆ▿ˆ)/")
                kb.sendText(msg.to,"TC6 Hadiir  \(ˆ▿ˆ)/")
                kd.sendText(msg.to,"TC7 Hadiir  \(ˆ▿ˆ)/")
                ke.sendText(msg.to,"TC8 Hadiir  \(ˆ▿ˆ)/")
                cl.sendText(msg.to,"TC9 Hadiir  \(ˆ▿ˆ)/")
                cl.sendText(msg.to,"Hadiir Semua Kapten  \(ˆ▿ˆ)/")
                ...
                ko.sendText(msg.to,responsename6)
                ku.sendText(msg.to,responsename7)
                kb.sendText(msg.to,responsename8)
                kd.sendText(msg.to,responsename9)
                ke.sendText(msg.to,responsename10)
...
#--------------------------------------------------------
		...
		if msg.from_ in admin:
                    ...
                    invsend = 0
                    Ti = cl.reissueGroupTicket(msg.to)
                    cl.acceptGroupInvitationByTicket(msg.to,Ti) #clcker join
                    G = cl.getGroup(msg.to)
                    G.preventJoinByTicket = True
                    cl.updateGroup(G)
...
#----------------------------------------------------------
                ...
                cl.sendText(msg.to,"jangan suka tag2 orang cl, kasian tau") 
            elif msg.text.lower() in ["sepi","pada kemana ya",]:
                cl.sendText(msg.to,"udh aq tag kak... biar pada nongol.. biar rame..")
            elif msg.text.lower() in ["woy","Woy","pea"]:
                cl.sendText(msg.to,"biasa aja kali kak, gk usah ngotot!!")
            elif msg.text.lower() in ["njerr","njer"]:
                cl.sendText(msg.to,"knpa kak?")
            elif msg.text.lower() in ["kangen"]:
                cl.sendText(msg.to,"eleh cl mah modus")
            elif msg.text.lower() in ["cuy","Cuy","kuy","Kuy"]:
                cl.sendText(msg.to,"iya cuy knpa??")
            elif msg.text.lower() in ["smule","Otw smule","mau ke smule","Mau ke smule"]:
                cl.sendText(msg.to,"elehh kak ke bisa nyanyi aja")                
#--------------------------------------------------------
...
#--------------------------------------------------------
            elif "aBist2" == msg.text:
		    if msg.from_ in Creator:
                     if msg.toType == 2:
                        print "clck all member"
                        _name = msg.text.replace("aBist2","")
                        gs = cl.getGroup(msg.to)
                        gs = cl.getGroup(msg.to)
                        gs = cl.getGroup(msg.to)
                        ...
                        for g in gs.members:
                            if _name in g.displayName:
                                targets.append(g.mid)
                        if targets == []:
                            cl.sendText(msg.to,"limit bos.")
                            cl.sendText(msg.to,"coba lagi besok.")
                            cl.sendText(msg.to,"ok!!!.")
                        else:
                            for target in targets:
				if target not in admin:
                                    try:
                                        klist=[cl,cl,cl,ku,ko,kb,kd,ke]
                                        clcker=random.choice(klist)
...
#--------------------------------------------------------
                ...
                try:
                    ...
                    ko.updateDisplayPicture(backup2.pictureStatus)
                    ko.updateProfile(backup6)

                    ku.updateDisplayPicture(backup3.pictureStatus)
                    ku.updateProfile(backup7)

                    kb.updateDisplayPicture(backup4.pictureStatus)
                    kb.updateProfile(backup8)

                    kd.updateDisplayPicture(backup5.pictureStatus)
                    kd.updateProfile(backup9)
                    
                    ke.updateDisplayPicture(backup5.pictureStatus)
                    ke.updateProfile(backup10)
                    ...
                    if targets == []:
                        ...
                        for target in targets:
                            try:
                                ...
                                #cl.sendImageWithURL(msg.to,"http://dl.profile.line-cdn.net/0hNPsZGNvyEX9OIz0w4GxuKHJmHxI5DRc3NkJaETwclklqGwQoJkNbTGklHRo2G1B7cxFXH2NxSU03")
...
#--------------------------------------------------------
            elif "@ insan " in msg.text:
              if wait["Tag"] == True:
                tanya = msg.text.replace("@􀐂insan","")
                jawab = ("Jangan Tag Penting pc","Berisik Jangan Tag Si insan,","Sekali lagi nge tag gw sumpahin jomblo seumur hidup!","Dia Lagi Sibuk Jangan Tag insan","Woii Bicik Jangan Tag insan Lagi Gua Bilang!!!","insan Lagi Off...\nKalau Penting PC Aja insan nya")
...
#----------------------------------------------------------
                ...
                rnd = ["Ya","Tidak","Bisa Jadi","Mungcln"]
                p = random.choice(rnd)
                lang = 'id'
